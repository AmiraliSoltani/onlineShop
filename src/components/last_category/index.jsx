...
const [filteredBySize, setFilteredBySize] = useState([]);
  ...
  // useEffect(() => {
  //   console.log("allllllllllllllllhiiiiiiiiiiiii")
  //   resetAndRecalculateFilters()
  ...
  useEffect(() => {
    ...
    if(refresh){
      console.log("allllllllllllllllhiiiiiiiiiiiii2222222222222222")
...
useEffect(() => {
  ...
  const resetAndRecalculateFilters = () => {
    // Reset all filters
    resetAllFilters();
...
const ImportInitialURL = () => {
  ...
  console.log("color2222222",currentColors)
    ...
    navigate(`/lastCategory/${categoryId}/${stringFilter}`);
...
const handleAllClick = (
  ...
  navigate(`/lastCategory/${categoryId}/${newStringFilter}`);
};
...
const importURlColors = (initialColors) => {
  let allColors = getRangeOfColor(); // Get all available colors
...
const getRangeOfColor = () => {
  ...
  console.log("searchWordMenu,","color",searchWordMenu)
  console.log("newcolors","final",final)

  let allColors = [];
  final.map((p) => allColors.push(...getcolors(p, allAttributeItems)));
  console.log("newcolors","allcolors",allColors)
...
const handelPriceClick = (indexOfPrice) => {
  ...
  navigate(`/lastCategory/${categoryId}/${finalFilterString}`);
...
{getParents().map((c) => {
    if (c.id !== categoryId) {
      ...
      <div  className={blurState.blur ? 'whole__category blurred' : 'whole__category'}>
        <div className="container">
...
//   // Check if any range is selected
//   const selectedRanges = updatedRangePriceClassName
//     .filter((range) => range.status) // Only include active ranges
//     .map((range) => ({
//       firtRange: range.firtRange,
//       secondRange: range.secondRange,
...
// const handleColor = (colorName, colorObject) => {
//   const updatedListOfColors = { ...listOfColors };
//   let updatedColors = [...colors];
